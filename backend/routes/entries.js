...
router.post("/", async (req, res) => {
  try {
    const { type, amount, category, note, datetime, image_url } =
      req.body || {};
    if (!type || !amount || !datetime)
...
router.put("/:id", async (req, res) => {
  try {
    ...
    for (const k of [
      ...
      if (k in req.body)
        patch[k] = k === "datetime" ? new Date(req.body[k]) : req.body[k];
    }
    if ("type" in patch)
      patch.type = patch.type === "income" ? "income" : "expense";
